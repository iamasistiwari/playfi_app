...
QUEUE_KEY = "song_tasks"   # Redis list for tasks

def worker():
    print("Worker started... waiting for tasks.")
    while True:
        # Blocking pop from Redis queue (BRPOP waits until a task is available)
        task = r.brpop(QUEUE_KEY, timeout=10)  
        if task:
            _, video_id = task
            print(f"Processing: {video_id}")

            # Fetch best audio stream URL
            musicUrl = getYoutubeMusicUrl(video_id)

            # Save result in Redis hash
            if musicUrl:
